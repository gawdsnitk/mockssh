...
if(isset($_GET['operation'])) {    
	...
	$fs = new fs(dirname(__FILE__) . DIRECTORY_SEPARATOR . 'data' . DIRECTORY_SEPARATOR ."$username" . DIRECTORY_SEPARATOR);
	try {
		$rslt = null;
...
<html>
	...
	<body>
		...
		<div id="container" role="main">
			...
			<button id= "view">Run</button>
			<div id="data">
				<div class="content code" style="display:none;"><div id="code"></div></div>
		...
		$(function () {
			var editor = ace.edit("code");
                  ...
                  //    var name = $(tree).jstree('get_selected');
		//	alert(name + "");
                      //  var x =  name.split('.')[1];
                       // alert(x + "");
                     /*   switch(x)
                        {       
                                case "php" : editor.getSession().setMode("ace/mode/php");
                                        break;
                                 case "js"  : editor.getSession().setMode("ace/mode/javascript");
                                        break;
                                default: editor.getSession().setMode("ace/mode/text");
                        }
*/
...
//				$("#code")		
				...
				$("#save").click(function(){
                                        ...
                                        //      data: {"target":"Hello" },
                                                data: {"datum":{
                                                                "filename": $('#tree').jstree('get_selected') + "",
...
alert("CHANGED!!");
			...
			//	dataType :"json", 
				...
				//	alert(data.response + "");
					...
					g_data = data;
			//		window.open('./data/' + data + '/' + name);
				   }
				 });
				window.open('./data/'+g_data +'/' + name);
                        ...
                        var name = $(tree).jstree('get_selected');
                        var x =  name.toString().split(".");
			var p = "Siddhant.txt";
			var y = p.toString().split(".")[1];	
		console.log(y);	
                     // alert(x.toString() + "");
                       /* switch(x)
